...
export const SELFTEST_HINTS = {
  ...
  'not found':
    'No bot exists with this token — usually a piece of it was lost while pasting. Re-paste TELEGRAM_BOT_TOKEN in full and redeploy.',
};
...
export function authorized(providedSecret, env = process.env) {
  const expected = cleanEnv('IP_HASH_SALT', env);
  return !!expected && String(providedSecret || '').trim() === expected;
}

export async function runSelftest({ send = false, env = process.env, fetchImpl = fetch } = {}) {
  const telegram = {
    configured: telegramConfigured(env),
    chatIdLooksNumeric: /^-?\d+$/.test(cleanEnv('TELEGRAM_CHAT_ID', env)),
  };
  const sheet = {
    configured: !!(cleanEnv('APPS_SCRIPT_WEBHOOK_URL', env) && cleanEnv('APPS_SCRIPT_SHARED_SECRET', env)),
    urlEndsWithExec: /\/exec$/.test(cleanEnv('APPS_SCRIPT_WEBHOOK_URL', env)),
  };
