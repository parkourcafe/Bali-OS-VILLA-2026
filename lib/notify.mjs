...
export function telegramConfigured(env = process.env) {
  return !!(cleanEnv('TELEGRAM_BOT_TOKEN', env) && cleanEnv('TELEGRAM_CHAT_ID', env));
}
...
export async function sendTelegram(payload, env = process.env, fetchImpl = fetch) {
  ...
  try {
    const res = await fetchImpl(`https://api.telegram.org/bot${cleanEnv('TELEGRAM_BOT_TOKEN', env)}/sendMessage`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        chat_id: cleanEnv('TELEGRAM_CHAT_ID', env),
...
export async function deliver(payload, { sheetSink, env = process.env, fetchImpl = fetch } = {}) {
  const sheetEnabled = !!(cleanEnv('APPS_SCRIPT_WEBHOOK_URL', env) && cleanEnv('APPS_SCRIPT_SHARED_SECRET', env));
