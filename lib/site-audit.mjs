...
export function aiChecks(html, robots) {
  const out = [];
  const jsonld = /<script[^>]+type=["']application\/ld\+json["']/i.test(html);
  out.push(jsonld
    ? pass('structured', 'ai', 'Structured data present', 'AI assistants and Google can read your business details reliably.')
    : warn('structured', 'ai', 'No structured data', 'Without schema.org markup, AI and Google guess at your prices, location and offering.'));

  const AI_BOTS = ['gptbot', 'oai-searchbot', 'claudebot', 'anthropic-ai', 'perplexitybot', 'google-extended'];
  ...
  out.push(blocked.length
    ? warn('ai-block', 'ai', 'AI crawlers are blocked', `robots.txt blocks: ${blocked.join(', ')}. AI assistants won't see this site.`)
    : pass('ai-block', 'ai', 'AI crawlers allowed', 'Major AI assistants are not blocked from reading the site.'));

  // JS-only heuristic: very little text in raw HTML → content is rendered client-side (bad for AI/Google).
  const text = html.replace(/<script[\s\S]*?<\/script>/gi, ' ').replace(/<style[\s\S]*?<\/style>/gi, ' ').replace(/<[^>]+>/g, ' ').replace(/\s+/g, ' ').trim();
  out.push(text.length >= 600
    ? pass('ssr', 'ai', 'Content readable without JavaScript', 'AI and search bots can read your content directly.')
    : warn('ssr', 'ai', 'Little content without JavaScript', 'Most text appears only after scripts run — AI crawlers often miss it.'));

  const h1 = /<h1[\s>]/i.test(html);
  out.push(h1
    ? pass('h1', 'ai', 'Main heading (H1) present', 'A clear headline helps machines understand the page topic.')
...
export async function auditSite(rawUrl, opts = {}) {
  ...
  const [pageR, robotsR, sitemapR] = await Promise.allSettled([
